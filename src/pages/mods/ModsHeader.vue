...
function toggleModTypes(modType: string) {
    // When "Only Select One Mod Type" is enabled, clicking a chip replaces the
    // selection with just that one (toggling off if it was already the only one).
                    ...
                    <template #default>
                        <div
                            ...
                            <div class="flex flex-col gap-1 text-md">
                                ...
                                <Checkbox v-model="filters.hideErrors" :label="$t('modsTab.header.advancedFilters.actions.hideErrorsMods')" />
                            </div>
                        </div>
                    </template>
                </Popover>

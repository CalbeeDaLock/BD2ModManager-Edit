...
<template>
    <Modal v-model:show="show" class="w-[50vw] max-h-[85vh]" @close="() => show = false">
        ...
        <div v-if="selectedNpc" class="flex flex-col min-h-0 text-text-primary overflow-hidden">
            <div class="flex items-stretch border-b border-border-default shrink-0">
                <Image :src="iconSrc" :alt="selectedNpc.name"
                    class="w-40 h-40 object-cover object-top shrink-0 border-r border-border-default aspect-square"
                ...
                <div class="flex-1 px-4 py-3">
                    <div class="flex items-start justify-between gap-2">
                        <div>
                            <div class="flex items-baseline gap-2">
                                <h3 class="font-semibold text-base text-text-primary">{{ selectedNpc.name }}</h3>
                            </div>
                        </div>
                    ...
                    <div class="flex gap-5 mt-3">
                        <div>
                            ...
                            <p class="text-xs text-text-secondary mt-0.5">{{ $t('npcTab.npcModal.enabledMods') }}</p>
                        </div>
                        <div>
                            <p class="text-base font-semibold leading-none">{{ installedMods.length }}</p>
                            <p class="text-xs text-text-secondary mt-0.5">{{ $t('npcTab.npcModal.totalMods') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col min-h-0 flex-1 overflow-y-auto">
                <div v-if="installedMods.length === 0" class="text-center py-12 px-4 text-text-secondary">
                    <p class="text-sm font-medium mb-1">{{ $t('npcTab.npcModal.noModsFound.title') }}</p>
                    <p class="text-xs text-text-secondary">{{ $t('npcTab.npcModal.noModsFound.description') }}</p>
                </div>
                <template v-else>
                    <label v-for="mod in sortedMods" :key="mod.name"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-text-secondary mt-0.5 font-mono truncate" :title="mod.path">{{ mod.path }}</p>
                        </div>
                    </label>
                </template>
            </div>
